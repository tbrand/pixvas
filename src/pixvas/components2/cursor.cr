module Pixvas
  class Cursor < Component
    ...
    def export : Bool
      return false if @mode == Mode::COMMAND
      context.command.set_command(Command::Type::EXPORT)
      mode_command
    end
    ...
    def input_command_char?(c) : Bool
      return false if c.ord > 122 || c.ord < 97
      context.command.add_char(@command_x, c)
      position(3 + (@command_x += 1), 2)
      true
    end
    ...
    def wait : Bool
      if c = STDIN.raw &.read_char
        ...
        return true if input_command_char?(c)
