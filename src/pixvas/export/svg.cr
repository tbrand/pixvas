module Pixvas
  class Svg < Export
    ...
    def rect(x : Int32, y : Int32, dot_width : Int32, cursor : Cursor) : String
      _x = UNIT_WIDTH * dot_width * x
      _y = UNIT_HEIGHT * y

      if color = cursor.pinned?(x, y)
        return "<rect x=\"#{_x}\" y=\"#{_y}\" width=\"#{UNIT_WIDTH * dot_width}\" height=\"#{UNIT_HEIGHT}\" fill=\"#{@color.hex(color)}\"/>"
      end
    ...
    def bg_rect(width, height) : String
      if color = @color.bg_color
        return "<rect x=\"0\" y=\"0\" width=\"#{width}\" height=\"#{height}\" fill=\"#{@color.hex(color)}\"/>"
      end

      ""
    end

    def export(width, height, dot_width, cursor : Cursor)
